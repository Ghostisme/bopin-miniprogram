...
import { fetchAiScripts, fetchWallet, generateAiScript, purchaseMembership, type AiScript, type Wallet } from '@/services'
...
export default function AIPage() {
  ...
  const load = async () => {
    ...
    try {
      const [walletData, scripts] = await Promise.all([fetchWallet(), fetchAiScripts()])
    ...
    <View className="ai-page">
      <View className="ai-page__hero">
        ...
        <View className="ai-page__quota">
          <View><Text className="ai-page__quota-value">{wallet?.aiQuota ?? '--'}</Text><Text>剩余 AI 次数</Text></View>
      ...
      <View className="ai-page__panel">
        ...
        <View className="ai-page__generate" onClick={handleGenerate}>{loading ? '正在生成...' : '生成话术'}</View>
      </View>
